...
import CircleOutlineIcon from '@assets/winote/illustration/circle-outline.svg?react';

const CATEGORY_BG: Record<string, string> = {
  Personal: 'bg-wn-card-green-light',
  Work: 'bg-wn-card-green',
  Business: 'bg-wn-card-yellow',
  Ideas: 'bg-wn-bg-light',
  Poetry: 'bg-wn-card-green-light',
};
...
export const NoteCard: React.FC<NoteCardProps> = ({ note, showBadge, variant = 'grid' }) => {
  ...
  const bgClass = CATEGORY_BG[note.category] || 'bg-wn-bg-light';
  ...
  return (
    <motion.button
      ...
      } shrink-0 rounded-wn-xl shadow-wn-card p-4 flex flex-col text-left cursor-pointer font-wn text-wn-text-primary overflow-hidden border-none ${bgClass}`}
    >
      {/* Time / Badge */}
      <div className="flex justify-between items-center mb-1.5">
        <span className={`text-[10px] text-wn-text-secondary ${showBadge ? 'font-bold' : 'font-medium'} leading-4 tracking-[0.3px]`}>
          {showBadge || (isCarousel ? formatDate(note.updatedAt) : formatTimeAgo(note.updatedAt))}
        </span>
        {note.isPinned && <Pin size={12} className="text-wn-text-secondary" />}
      </div>

      {/* Title */}
      <h3 className={`${isCarousel ? 'text-wn-lg' : 'text-[17px]'} font-semibold leading-[22px] mb-2`}>
        {note.title}
      </h3>
        ...
        <ul className="list-none text-wn-xs text-wn-text-secondary flex-1">
          {note.checklist.slice(0, 3).map((item) => (
            <li key={item.id} className="flex items-center gap-1.5 mb-1">
              <CircleOutlineIcon className="w-3 h-3 shrink-0 text-wn-text-secondary" />
      ...
      {note.type === 'text' && note.content && (
        <p
          className="text-wn-xs text-wn-text-secondary leading-[18px] flex-1 overflow-hidden"
          style={{
            display: '-webkit-box',
      ...
      {note.type === 'audio' && (
        <div className="flex items-center gap-2 mt-1 flex-1">
          <div className="w-7 h-7 rounded-full bg-wn-cta-bg flex items-center justify-center">
      ...
      {note.type === 'image' && (
        <div className="flex-1 bg-wn-card-lavender rounded-wn-sm flex items-center justify-center min-h-[60px] mb-1">
          <Image size={24} className="text-wn-text-tertiary" />
        </div>
      )}
